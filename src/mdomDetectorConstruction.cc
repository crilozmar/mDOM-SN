...
G4double MPMTroffset = 2.6*mm;		// middle PMT circles are slightly further out due to MPMTzoffset
	...
	//	hard-coded limits
const double PHOTON_NRG_MAX = (hc_eVnm / 200)*eV;
const double PHOTON_NRG_MIN = (hc_eVnm / 720)*eV;
...
//	Mie Scattering
	...
	//G4int Depth_pos = 65; G4cout << "BE AWARE WE ARE IN THE DIRTIEST ICE!!!" << G4endl;
...
// ------------------------- choosing glass for simulation ------------------------------------
	...
	proptable_KoppGlass->AddProperty("RINDEX", GeneralRIndPhotonEnergy, ChibaGlassRInd, 49);
 	proptable_KoppGlass->AddProperty("ABSLENGTH", IceCubeGlassPhotonEnergy, IceCubeGlassAbsLen,36);
...
// 	G4double PMT_12199_SphereAngle = 50*degree; //explicit declaration
	...
	PMT_12199_bulk_solid = new G4Tubs("PMT_12199 solid bulk", 0.0, 0.5*51.9*mm, (97-23.3)*0.5*mm, 0, 2*pi);
...
// 	G4double PC_12199_ShieldRad = 40.*mm * std::sqrt(1.-std::pow((PC_12199_CathodeLimit - PC_12199_ShieldToCathode)/(21.5*mm), 2.))-1*mm;	// automatic shield diameter based on ellipsoid formula, with radial offset
...
// 	G4double PC_ETEL_ShieldRad = (0.5*86.5)*mm * std::sqrt(1.-std::pow((PC_ETEL_CathodeLimit - PC_ETEL_ShieldToCathode)/(26.2*mm), 2.))-1*mm;	// automatic shield diameter based on bulb ellipsoid formula with radial offset, 
...
// 	PMT center = center of sphere (PMT10_solid_1)
	...
	PC10_physical = new G4PVPlacement (0, G4ThreeVector(0,0,-2*mm), PC10_logical, "10in photocathode", PMT10_tube_logical, false, 0);
...
// ------------------- optical border surfaces --------------------------------------------------------------------------------
	...
	if (OM_type == "PDOM") {
		...
		G4Ellipsoid* PDOM_AirAux_solid = new G4Ellipsoid("PDOM_AirAux solid", 0.5*12*25.4*mm, 0.5*12*25.4*mm, 0.5*12*25.4*mm, -0.5*13*25.4*mm, 50*mm);
...
// 		rot->rotateY(180*deg);
		...
		PDOM_Air_physical = new G4PVPlacement (rot, G4ThreeVector(0,0,0), PDOM_Air_logical, "PDOM_Air physical", PDOM_Gel_logical, false, 0);
		
		PMT_physical[0] = new G4PVPlacement (rot, G4ThreeVector(0,0, PDOM_PMT_z), PMT10_tube_logical, "PMT_0_physical", PDOM_Gel_logical, false, 0);
		...
		PDOM_Glass_physical = new G4PVPlacement (0, G4ThreeVector(0,0,0), PDOM_Glass_logical, "PDOM_Glass physical", World_logical, false, 0);
